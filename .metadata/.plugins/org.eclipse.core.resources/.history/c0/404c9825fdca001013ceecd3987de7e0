...
public class ManipuladorArquivoEstoque {
	 ...
	 model.factories.FabricaAbstrata fabricaShorts = new model.factories.FabricaShortsJeans();
	   ...
	   // public void atualizarEstoque(String nomeProduto, int venda) {
	     //   for (ProdutoAbstrato p : produtoPesquisado) 
	        
	    public ProdutoAbstrato atualizarEstoque(String nomeProd, int venda) {
	        for (ProdutoAbstrato p : produtoPesquisado) {
	            if (p.getNome().equalsIgnoreCase(nomeProd)){
	            	int novaQuantidade = p.getQtdEstoque() - venda;
	            	p.setQtdEstoque(novaQuantidade);
	                produtoPesquisado.set(produtoPesquisado.indexOf(p), p);
	                limparEstoqueTxt();
	                break;
	            } else {
	    ...
	    public void limparEstoqueTxt() {
	        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH))) {
	            writer.write(""); 
	        } catch (IOException e) {
	            throw new RuntimeException("Erro ao limpar arquivo do estoque!", e);
	        }
	    }
